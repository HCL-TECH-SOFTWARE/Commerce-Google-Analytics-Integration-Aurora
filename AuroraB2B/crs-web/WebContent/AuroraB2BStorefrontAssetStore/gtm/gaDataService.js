...
const gaDataService = {
		...
		setBreadCrumbArr(arr,lastcategory){			
			...
			console.log("breadcumb--",breadcrumbArr);
		},
		...
		setUserDetail(logonStatus, userId) {
			...
			userID = userId;
			console.log("user detail--",logonStatus,userId);
		},

		getUserDetail() {
			return {
				login: logonState === 1 ? "logged In" : "logged Out",
						userID: userID
			};
		},
		...
		sendDataItems(pgorder,itemdetails){
			console.log("pg--",pgorder);
			sessionStorage.setItem("pgorder",JSON.stringify(pgorder));
			sessionStorage.setItem("itemdetail",JSON.stringify(itemdetails));

		},
		getproductsInfo(){
			pgorder = JSON.parse(sessionStorage.getItem("pgorder"));
			itemdetails=JSON.parse(sessionStorage.getItem("itemdetail"));
			var cartItemsArr = [];
		...
		sendB2BPDPDetailViewEvent(currentProdSelect) {
			const obj = {
		...
		sendAddToCartEvent(currentProdSelect) {
			console.log("crat--",currentProdSelect);
			if(skulist.length !== 0){
				this.sendB2BAddToCartEvent(currentProdSelect[0],skulist);
			}else{
		...
		sendB2BAddToCartEvent(currentProdSelect, result) {
			const price = currentProdSelect.productPrice;
			const productObj = [];
			result.map((element) => {
				const obj = {
						id: element.id,
		...
		sendCheckoutEvent(step, value) {
			var productArr = this.getproductsInfo();
		...
		sendPurchaseEvent() {
			...
			const obj = {
					...
					discount: coupon,
					currency: currency,
					cartItemsArr:this.getproductsInfo(),
			};
