...
$(document).ready(function(){
	gaDataService.sendPDPPageViewEvent();
});

window.addEventListener('load', (event) => {
  	//console.log('page is fully loaded', $('#container_${pageDesign.layoutId} span[id^="product_SKU_"]')[0].textContent.slice(4));
  	var price = $('#container_${pageDesign.layoutId} input[id^="ProductInfoPrice_"]')[0].value.slice(1);	
	var sku = $('#container_${pageDesign.layoutId} span[id^="product_SKU_"]')[0].textContent.slice(4);	
	var product = {
		price,
		sku
	};
	gaDataService.sendB2BPDPDetailViewEvent(product);
});
