		...
		<span class="text">
			...
			<span class="paging">
				...
				<c:if test="${beginIndex != 0}">
					...
					wcRenderContext.updateRenderContext('traditionalShipmentDetailsContext',{'beginIndex':'<c:out value='${beginIndex - pageSize}'/>','pageSize':'<c:out value='${pageSize}'/>','initializeJS':'true'});}}">
				</c:if>
				...
				<c:if test="${numEntries > endIndex }">
					...
					wcRenderContext.updateRenderContext('traditionalShipmentDetailsContext',{'beginIndex':'<c:out value='${beginIndex + pageSize}'/>','pageSize':'<c:out value='${pageSize}'/>','pageSize':'<c:out value='${pageSize}'/>','initializeJS':'true'});}}">
				</c:if>
...
gaDataService.sendDataItems(<wcf:json object="${pgorder.orderItem}" />,<wcf:json object="${itemDetailsInThisOrder}" />);
});
